...
function canPlayH265(videoTestElement, options) {
    ...
    if (browser.chromecast) {
        const isChromecastUltra = userAgent.indexOf('aarch64') !== -1;
...
export default function BrowserDeviceProfile(options) {
    options = options || {};
    ...
    const canPlayAv1 = videoTestElement.canPlayType('video/webm; codecs="av1"').replace(/no/, '');
    ...
    const supportsMp2VideoAudio = browser.edgeUwp || browser.tizen || browser.orsay || browser.web0s;
    ...
    let supportsDts = browser.tizen || browser.orsay || browser.web0s || options.supportsDts;
    ...
    const supportsSecondaryAudio = browser.tizen || browser.orsay || videoTestElement.audioTracks;

    let aacCodecProfileConditions = [];
